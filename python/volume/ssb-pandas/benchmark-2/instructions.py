...
def filter_field_ge(field:str, value) -> Callable[[dict[str, DataFrame]], None]:
    def filter(dfs: dict[str, DataFrame]) -> None:
        ...
        table = dfs[name]
        del dfs[name]
        dfs[f'({name}𝜎{field}>{value})'] = table.loc[table[field].isin(value)]
        
    return filter
def filter_field_le(field:str, values:list[str]) -> Callable[[dict[str, DataFrame]], None]:
    def filter(dfs: dict[str, DataFrame]) -> None:
        name = find_table(dfs, field)
        table = dfs[name]
        del dfs[name]
    ...
    return filter
def filter_field_lt(field:str, values:list[str]) -> Callable[[dict[str, DataFrame]], None]:
    def filter(dfs: dict[str, DataFrame]) -> None:
        name = find_table(dfs, field)
        table = dfs[name]
        del dfs[name]
