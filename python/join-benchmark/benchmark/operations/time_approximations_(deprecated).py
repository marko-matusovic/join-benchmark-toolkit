...
class Approx_Instructions(Operations[TData, float]):
    ...
    def join_fields(self, field_name_1: str, field_name_2: str):
        def join(data: TData) -> float:
            ...
            data["schema"][res] = data["schema"][name_1] + \
                data["schema"][name_2]
            ...
            if res not in data["times"]:
                sm = min(data["stats"][name_1]["length"],
                         data["stats"][name_2]["length"])
                bg = max(data["stats"][name_1]["length"],
                         data["stats"][name_2]["length"])
    ...
    def filter_eq(self, data: TData, field: str, values: list[TVal]):
        ...
        if res not in data["times"]:
            reduction = 1.0 * len(values) / \
                data["stats"][name]["unique"][field]
            data["times"][res] = (
                data["stats"][name]["length"] ** TIME_COMP_POW) * TIME_COMP
            data["stats"][res] = {
