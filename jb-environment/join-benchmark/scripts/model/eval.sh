...
# 1st {{model1,model2: list[str]}}  -> model(s) to train on
# 2nd {{model3: str}}               -> model to evaluate
# 3rd {{set: int}}                  -> the set number
...
MODELS_TO_TRAIN_CM="$1"                        # separated with commas
...
if [[ (! -z "$CLS_OPTION") ]]; then
    ML="cls"
    MODEL_NAME="gbdt_cls/set_${SET_NUMBER}_NJO${CLS_OPTION}_${MODELS_TO_TRAIN_US}"
    MODEL_PATH="${CURRENT_DIR}/../../results/models/${MODEL_NAME}.pickle"
    TRAIN_OPTION="--num-joins ${CLS_OPTION}"
fi
if [[ (! -z "$REG_OPTION") ]]; then
    ML="reg"
    MODEL_NAME="gbdt_reg/set_${SET_NUMBER}_BS${REG_OPTION}_${MODELS_TO_TRAIN_US}"
    MODEL_PATH="${CURRENT_DIR}/../../results/models/${MODEL_NAME}.pickle"
...
if [[ $TRAIN == true ]] || [[ ! -f "$MODEL_PATH" ]]; then
    python3 main.py train-${ML} ${MODELS_TO_TRAIN_CM} ${SET_NUMBER} ${TRAIN_OPTION}
fi

python3 main.py eval-${ML} ${MODEL_TO_EVAL} ${SET_NUMBER} ${MODEL_NAME}
