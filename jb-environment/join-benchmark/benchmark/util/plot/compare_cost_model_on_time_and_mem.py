...
def calc_print_plot_bins(real_data, cost_model_data, label, unit):
    ...
    cost_model_data_bins = [
        [cost_model_data[perm] for perm in perm_bin] for perm_bin in perm_bins
    ]
    ...
    for b in range(BINS):
        ...
        print(
            f'| {"Real "+label:>25} | {np.mean(real_data_bins[b]):30.8f} | {np.std(real_data_bins[b]):30.8f} |'
        )
        print(
            f'| {"Cost "+label:>25} | {np.mean(cost_model_data_bins[b]):30.8f} | {np.std(cost_model_data_bins[b]):30.8f} |'
        )
    print("-" * 95)
    ...
    plt.savefig(
        f"results/figs/cost-comparison/gpu/{query}/{label.lower()}.png", dpi=250
    )
...
if __name__ == "__main__":
    ...
    for i in df_time.T:
        ...
        try:
            df_mem = pd.read_csv(
                f"results/ncu/{query}/{perm}.csv", sep=",", comment="="
            )
