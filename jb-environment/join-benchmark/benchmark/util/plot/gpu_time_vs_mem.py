...
if __name__ == "__main__":
    ...
    for rw in ["read", "write"]:
        for item in ["sum", "max"]:
            ...
            plt.title(
                f"Execution Time vs DRAM {rw.capitalize()} {item.capitalize()}\nper Random Join Orders of {query}"
            )
            plt.xlabel("time [s]")
            plt.ylabel("memory [B]")
            ...
            points = np.array(
                [
                    [stats[perm]["time"]["mean"], stats[perm]["memory"][rw][item]]
                    for perm in stats
                ]
            )
            ...
            plt.savefig(
                f"results/figs/gpu/TvM/{query}/time-mean-vs-mem-{rw}-{item}.png"
            )
