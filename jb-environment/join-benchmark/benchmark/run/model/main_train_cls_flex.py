...
def main(
    ...
    for db_set in db_sets:
        ...
        (Xi, yi) = encode_all_cls(features, measurements, None, True, True)
        print(f"{db_set} has {len(Xi)} training entries")
        if len(Xi) == 0:
            continue
        X += Xi
        y += yi
    ...
    file = f"{res_path}/models/cls_flex/{ml_model}/set_{training_set}_{'_'.join(db_sets)}.pickle"
    ensure_dir(file)
